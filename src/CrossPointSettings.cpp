...
uint8_t CrossPointSettings::writeSettings(FsFile& file, bool count_only) const {
  ...
  writer.writeItem(file, blePageTurnerEnabled);
  writer.writeItemString(file, bleDeviceAddress);
...
bool CrossPointSettings::loadFromFile() {
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    serialization::readPod(inputFile, blePageTurnerEnabled);
    if (++settingsRead >= fileSettingsCount) break;
    {
      std::string addrStr;
      serialization::readString(inputFile, addrStr);
      strncpy(bleDeviceAddress, addrStr.c_str(), sizeof(bleDeviceAddress) - 1);
      bleDeviceAddress[sizeof(bleDeviceAddress) - 1] = '\0';
    }
    if (++settingsRead >= fileSettingsCount) break;
